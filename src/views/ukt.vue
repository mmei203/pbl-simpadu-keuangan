  ...
  <div class="main-content">
    <header class="topbar">
      <div>
        ...
        <h1>Kelola UKT</h1>
        <p class="subtitle">
          Perbarui dan pantau data UKT mahasiswa secara real-time
        </p>
      </div>
    </header>
            ...
            <tr v-for="(item, index) in dataMahasiswa" :key="item.id || index">
              ...
              <td>
                <div class="jurusan-text">
                  {{ item.jurusan?.name || "Teknik Elektro" }}
                </div>
                <div class="prodi-text">
                  {{ item.prodi?.name || "D3 Teknik Informatika" }}
                </div>
              </td>
    ...
    if (dataMahasiswa.value && Array.isArray(dataMahasiswa.value)) {
      ...
      if (found) {
        const info = {
          nama:
            found.nama ||
            found.nama_mahasiswa ||
            found.NAMA ||
            tableData.value[idx].nama,
...
const fetchDataUkt = async () => {
  ...
  errorMessage.value = "";
  try {
    const response = await apiKeuangan.get(
      `https://api-keuangan-4a.akufarish.my.id:8873/api/keuangan-mahasiswa/`,
      {
        params: {
    ...
    if (error.response && error.response.status === 429) {
      errorMessage.value =
        "Error Server (429): Terlalu banyak request. Silakan tunggu semenit.";
...
const editUkt = (item) => {
  alert(
    `Mode Edit UKT untuk: ${item.nama || "-"}\nGolongan Saat Ini: Golongan ${item.golongan_ukt || "-"}`,
  );
};

async function getDataMahasiswa() {
  try {
    // 1. Ambil data mahasiswa dan data prodi secara bersamaan menggunakan Promise.all
    const [resMahasiswa, resProdi] = await Promise.all([
      ...
      apiKeuangan.get(`https://be.karlearn.site/api/prodi`, {
        timeout: 4000,
        headers: { Accept: "application/json" },
      }),
    ]);

    const listMahasiswa = resMahasiswa.data.data; // Berupa Array dari gambar
    const listProdi = resProdi.data.data; // Berupa Array prodi

    // 2. Gabungkan data berdasarkan prodi_id
    const dataGabungan = listMahasiswa.map((mahasiswa) => {
      // Cari prodi yang memiliki id sama dengan prodi_id milik mahasiswa
      const prodiDitemukan = listProdi.find((p) => p.id === mahasiswa.prodi_id);

      return {
        ...mahasiswa,
        // Jika prodi ditemukan, masukkan datanya. Jika tidak, beri nilai null.
        prodi: prodiDitemukan
          ? {
              id: prodiDitemukan.id,
              name: prodiDitemukan.name,
              jenjang: prodiDitemukan.jenjang,
            }
          : null,
        jurusan: prodiDitemukan ? prodiDitemukan.jurusan : null,
      };
    });

    console.log(dataGabungan);

    // 3. Simpan ke reactive variable Vue kamu
